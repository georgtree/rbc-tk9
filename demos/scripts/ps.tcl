...
proc ::rbc::ps::CreateOutline {canvas} {
    ...
    $canvas bind image <ButtonPress-1> [list ::rbc::ps::StartMove $canvas %x %y]
    $canvas bind image <B1-Motion> [list ::rbc::ps::DragMove $canvas %x %y]
    $canvas bind image <Shift-B1-Motion> [list ::rbc::ps::DragMoveBut $canvas %x %y]
    $canvas bind image <ButtonRelease-1> [list ::rbc::ps::EndMove $canvas]
    $canvas bind image <Enter> [list ::rbc::ps::EnterImage $canvas]
    $canvas bind image <Leave> [list ::rbc::ps::LeaveImage $canvas]
    focus $canvas
    $canvas create rectangle $xMin $yMin [expr {$xMin+$gripSize}] [expr {$yMin+$gripSize}] -tags {outline grip nw} 
    $canvas create rectangle [expr {$xMax-$gripSize}] [expr {$yMax-$gripSize}] $xMax $yMax -tags {outline grip se}
    $canvas create rectangle [expr {$xMax-$gripSize}] [expr {$yMin+$gripSize}] $xMax $yMin -tags {outline grip ne}
    $canvas create rectangle $xMin $yMax [expr {$xMin+$gripSize}] [expr {$yMax-$gripSize}] -tags {outline grip sw}
    set xMid [expr {($xMax+$xMin-$gripSize)*0.5}]
    set yMid [expr {($yMax+$yMin-$gripSize)*0.5}]
    $canvas create rectangle $xMid $yMin [expr {$xMid+$gripSize}] [expr {$yMin+$gripSize}] -tags {outline grip n}
    $canvas create rectangle $xMid [expr {$yMax-$gripSize}] [expr {$xMid+$gripSize}] $yMax -tags {outline grip s}
    $canvas create rectangle [expr {$xMax-$gripSize}] $yMid $xMax [expr {$yMid+$gripSize}] -tags {outline grip e}
    $canvas create rectangle $xMin $yMid [expr {$xMin+$gripSize}] [expr {$yMid+$gripSize}] -tags {outline grip w}
    foreach grip {e w s n sw ne se nw} {
        $canvas bind $grip <ButtonPress-1> [list ::rbc::ps::StartResize %W $grip %x %y]
        $canvas bind $grip <B1-Motion> [list ::rbc::ps::DragResize %W %x %y]
        $canvas bind $grip <Shift-B1-Motion> [list ::rbc::ps::DragResizeBut %W %x %y]
        $canvas bind $grip <ButtonRelease-1> [list ::rbc::ps::EndResize %W $grip %x %y]
        $canvas bind $grip <Enter> [list ::rbc::ps::EnterGrip %W $grip %x %y]
        $canvas bind $grip <Leave> [list ::rbc::ps::LeaveGrip %W $grip]
    }
    $canvas raise grip
    ...
    set pageInfo(image) [image create photo -width [expr {int($xMax-$xMin)}] -height [expr {int($yMax-$yMin)}]]
...
proc ::rbc::ps::AdjustCanvas {canvas} {
    ...
    $pageInfo(image) configure -width  [expr {int($pageInfo(xMax)-$pageInfo(xMin))}]\
            -height [expr {int($pageInfo(yMax)-$pageInfo(yMin))}]
    winop image resample $pageInfo(largeImage) $pageInfo(image) sinc
    $canvas coords nw $xMin $yMin [expr {$xMin+$gripSize}] [expr {$yMin+$gripSize}]
    $canvas coords se [expr {$xMax-$gripSize}] [expr {$yMax-$gripSize}] $xMax $yMax 
    $canvas coords ne [expr {$xMax-$gripSize}] [expr {$yMin+$gripSize}] $xMax $yMin 
    $canvas coords sw $xMin $yMax [expr {$xMin+$gripSize}] [expr {$yMax-$gripSize}]
    $canvas coords n $xMid $yMin [expr {$xMid+$gripSize}] [expr {$yMin+$gripSize}]
    $canvas coords s $xMid [expr {$yMax-$gripSize}] [expr {$xMid+$gripSize}] $yMax
    $canvas coords e [expr {$xMax-$gripSize}] $yMid $xMax [expr {$yMid+$gripSize}]
    $canvas coords w $xMin $yMid [expr {$xMin+$gripSize}] [expr {$yMid+$gripSize}]
    return
}
...
proc ::rbc::ps::CanvasToPs {} {
    ...
    set padXR [expr {($pageInfo(paperWidth)-$pageInfo(xMax))/$pageInfo(scale)}]
    set padYU [expr {$pageInfo(yMin)/$pageInfo(scale)}]
    set padYD [expr {($pageInfo(paperHeight)-$pageInfo(yMax))/$pageInfo(scale)}]
    if {$pageInfo(-landscape)} {
        set pageInfo(-padx) [list [ConvertUnits $padYU] [ConvertUnits $padYD]]
        set pageInfo(-pady) [list [ConvertUnits $padXR] [ConvertUnits $padXL]]
    } else {
        set pageInfo(-padx) [list [ConvertUnits $padXL] [ConvertUnits $padXR]]
        set pageInfo(-pady) [list [ConvertUnits $padYU] [ConvertUnits $padYD]]
    }
...
proc ::rbc::ps::EnterImage  {canvas} {
    ...
    bind $canvas <KeyPress-Left> {
        ::rbc::ps::DragMove %W [expr {$::rbc::ps::pageInfo(lastX)-1}] $::rbc::ps::pageInfo(lastY)
    }
    bind $canvas <KeyPress-Right> {
        ::rbc::ps::DragMove %W [expr {$::rbc::ps::pageInfo(lastX)+1}] $::rbc::ps::pageInfo(lastY)
    }
    bind $canvas <KeyPress-Up> {
        ::rbc::ps::DragMove %W $::rbc::ps::pageInfo(lastX) [expr {$::rbc::ps::pageInfo(lastY)-1}]
    }
...
proc ::rbc::ps::EnterGrip {canvas grip x y} {
    ...
    bind $canvas <KeyPress-Left> {
        ::rbc::ps::DragResize %W [expr {$::rbc::ps::pageInfo(lastX)-1}] $::rbc::ps::pageInfo(lastY)
    }
    bind $canvas <KeyPress-Right> {
        ::rbc::ps::DragResize %W [expr {$::rbc::ps::pageInfo(lastX)+1}] $::rbc::ps::pageInfo(lastY)
    }
    bind $canvas <KeyPress-Up> {
        ::rbc::ps::DragResize %W $::rbc::ps::pageInfo(lastX) [expr {$::rbc::ps::pageInfo(lastY)-1}]
    }
    bind $canvas <KeyPress-Down> {
        ::rbc::ps::DragResize %W $::rbc::ps::pageInfo(lastX) [expr {$::rbc::ps::pageInfo(lastY)+1}]
    }
...
proc ::rbc::ps::DragMoveBut {canvas x y} {
    ...
    if {$pageInfo(direction) eq {undecided}} {
        if {abs($dx)>abs($dy)} {
...
proc ::rbc::ps::DragResizeBut {canvas x y} {
    ...
    set buff [expr {$gripSize*5}]
    set aspect [expr {[winfo fpixels $graph [$graph cget -height]]/[winfo fpixels $graph [$graph cget -width]]}]
    switch $pageInfo(grip) {
        nw -
        n {
