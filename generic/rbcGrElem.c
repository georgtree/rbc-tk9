    ...
    Rbc_Uid classUid)
{
    ...
    if (elemName[0] == '-') {
	Tcl_AppendResult(graphPtr->interp, "name of element \"", elemName,
			 "\" can't start with a '-'", (char *)NULL);
	return TCL_ERROR;
    }
    hPtr = Tcl_CreateHashEntry(&graphPtr->elements.table, elemName, &isNew);
    if (!isNew) {
	Tcl_AppendResult(interp, "element \"", elemName,
    ...
    if (Rbc_ConfigureWidgetComponent(interp, graphPtr->tkwin, elemPtr->name,
	    "Element", elemPtr->specsPtr, objc-4, objv+4, (char *)elemPtr, 0) != TCL_OK) {
	DestroyElement(graphPtr, elemPtr);
	return TCL_ERROR;
    }
    ...
    Tcl_Obj *const objv[])
{
	...
	objv+6, &search, TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	 ...
	 */
	if (Tcl_ObjSetVar2(interp, objv[5], Tcl_NewStringObj("name", -1),
		Tcl_NewStringObj(search.elemPtr->name, -1), flags) == NULL) {
	    return TCL_ERROR;
	}
	if (Tcl_ObjSetVar2(interp, objv[5], Tcl_NewStringObj("index", -1),
		Tcl_NewIntObj(search.index), flags) == NULL) {
	    return TCL_ERROR;
	}
	if (Tcl_ObjSetVar2(interp, objv[5], Tcl_NewStringObj("x", -1),
		Tcl_NewDoubleObj(search.point.x), flags) == NULL) {
	    return TCL_ERROR;
	}
	if (Tcl_ObjSetVar2(interp, objv[5], Tcl_NewStringObj("y", -1),
		Tcl_NewDoubleObj(search.point.y), flags) == NULL) {
	    return TCL_ERROR;
	}
	if (Tcl_ObjSetVar2(interp, objv[5], Tcl_NewStringObj("dist", -1),
		Tcl_NewDoubleObj(search.dist), flags) == NULL) {
	    return TCL_ERROR;
	}
