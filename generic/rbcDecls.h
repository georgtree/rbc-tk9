...
/* 0 */
RBCAPI int		Rbc_CreateVector(Tcl_Interp *interp,
				const char *vecName, Tcl_Size size,
				Rbc_Vector **vecPtrPtr);
/* 1 */
RBCAPI int		Rbc_GetVector(Tcl_Interp *interp,
				const char *vecName, Rbc_Vector **vecPtrPtr);
/* 2 */
RBCAPI int		Rbc_ResizeVector(Rbc_Vector *vecPtr,
				Tcl_Size nValues);
/* 3 */
RBCAPI char *		Rbc_NameOfVector(Rbc_Vector *vecPtr);
/* 4 */
RBCAPI int		Rbc_ResetVector(Rbc_Vector *vecPtr, double *dataArr,
				Tcl_Size nValues, Tcl_Size arraySize,
				Tcl_FreeProc *freeProc);
/* 5 */
RBCAPI double *		Rbc_VectorData(Rbc_Vector *v);
/* 6 */
RBCAPI Tcl_Size		Rbc_VectorLength(Rbc_Vector *v);
/* 7 */
RBCAPI Tcl_Size		Rbc_VectorSize(Rbc_Vector *v);
/* 8 */
...
typedef struct RbcStubs {
    ...
    int (*rbc_CreateVector) (Tcl_Interp *interp, const char *vecName, Tcl_Size size, Rbc_Vector **vecPtrPtr); /* 0 */
    int (*rbc_GetVector) (Tcl_Interp *interp, const char *vecName, Rbc_Vector **vecPtrPtr); /* 1 */
    int (*rbc_ResizeVector) (Rbc_Vector *vecPtr, Tcl_Size nValues); /* 2 */
    char * (*rbc_NameOfVector) (Rbc_Vector *vecPtr); /* 3 */
    int (*rbc_ResetVector) (Rbc_Vector *vecPtr, double *dataArr, Tcl_Size nValues, Tcl_Size arraySize, Tcl_FreeProc *freeProc); /* 4 */
    double * (*rbc_VectorData) (Rbc_Vector *v); /* 5 */
    Tcl_Size (*rbc_VectorLength) (Rbc_Vector *v); /* 6 */
    Tcl_Size (*rbc_VectorSize) (Rbc_Vector *v); /* 7 */
