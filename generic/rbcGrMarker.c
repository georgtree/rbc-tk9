    ...
    Marker *markerPtr)
{
	...
	photo = Tk_FindPhoto(graphPtr->interp,
...
//    char c;
    ...
    if ( Tcl_GetIndexFromObjStruct(interp, objv[3], typeMap,
	    sizeof(struct markerTypeMap), "marker type", 0, &index) != TCL_OK) {
	return TCL_ERROR;
    }
...
#define FIND_OVERLAPPING (1<<1)
	...
	{"enclosed",    FIND_ENCLOSED},
	{"overlapping", FIND_OVERLAPPING},
	{NULL, 0}
    };
    int index;

    if ( Tcl_GetIndexFromObjStruct(interp, objv[3], searchMap,
	    sizeof(struct markerSearchMap), "search type", 0, &index) != TCL_OK) {
	return TCL_ERROR;
    }
...
static Rbc_OpSpec markerOps[] = {
    {"after",     (Rbc_Op)RelinkOp,    4, 5, "marker ?afterMarker?"},
