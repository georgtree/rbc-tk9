    ...
    Tcl_Obj *const objv[])
{
    ...
    if (nOpts == 0) {
        ...
        return Tk_ConfigureInfo(interp, graphPtr->tkwin,
        penPtr->configSpecs, (char *)penPtr, Tcl_GetString(options[0]),
        flags);
    }
    ...
    Tcl_Obj *const objv[])
{
    ...
    penPtr = Rbc_CreatePen(graphPtr, Tcl_GetString(objv[3]),
    graphPtr->classUid, objc-4, objv+4);
    ...
    Tcl_Obj *const objv[])
{
    ...
    if (penPtr->flags & PEN_DELETE_PENDING) {
        Tcl_AppendResult(graphPtr->interp, "can't find pen \"",
...
static Rbc_OpSpec penOps[] = {
    {"cget",      (Rbc_Op)CgetOp,      5, 5, "penName option"},
