...
static Rbc_OpSpec splineOps[] = {
    {"natural",   (Rbc_Op)Rbc_NaturalSpline,   6, 6, "x y splx sply"},
    ...
    Tcl_Obj *const objv[])
{
    ...
    if ((Rbc_GetVector(interp, xName, &x) != TCL_OK) ||
        (Rbc_GetVector(interp, yName, &y) != TCL_OK) ||
        (Rbc_GetVector(interp, splXName, &splX) != TCL_OK)) {
    return TCL_ERROR;
    }
    nOrigPts = Rbc_VecLength(x);
    if (nOrigPts < 3) {
    Tcl_AppendResult(interp, "length of vector \"", xName, "\" is < 3",
        (char *)NULL);
    return TCL_ERROR;
    }
    for (i = 1; i < nOrigPts; i++) {
    if (Rbc_VecData(x)[i] < Rbc_VecData(x)[i - 1]) {
        Tcl_AppendResult(interp, "x vector \"", xName,
    ...
    Tcl_AppendResult(interp, "vectors \"", xName, "\" and \"", yName,
...
Rbc_SplineInit(interp)
    Tcl_Interp *interp;
{
    Tcl_CreateObjCommand(interp, "::rbc::spline", SplineObjCmd,
