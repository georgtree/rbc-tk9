# RBC.graph.marker.configure.bitmap.M.test --
#
###Abstract
# This file contains the manual tests that test the bitmap marker configure
# function of the graph BLT component.
#
###Copyright
# See the file "license.terms" for information on usage and
# redistribution of this file, and for a DISCLAIMER OF ALL WARRANTIES.
#
###Revision String
# SVN: $Id$

###Package Definition
package provide graph.marker

package require rbc
namespace import rbc::*

namespace eval graph.marker {
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring background changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.1.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.1.1.Body {} {
		.graph1 marker configure marker1 -background red
	}
	
	proc RBC.graph.marker.configure.bitmap.M.1.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring bitmap changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.2.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.2.1.Body {} {
		.graph1 marker configure marker1 -bitmap @greenback.xbm
	}
	
	proc RBC.graph.marker.configure.bitmap.M.2.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring fill changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.3.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.3.1.Body {} {
		.graph1 marker configure marker1 -fill red
	}
	
	proc RBC.graph.marker.configure.bitmap.M.3.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring foreground changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.4.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.4.1.Body {} {
		.graph1 marker configure marker1 -foreground red
	}
	
	proc RBC.graph.marker.configure.bitmap.M.4.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring outline changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.5.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.5.1.Body {} {
		.graph1 marker configure marker1 -outline red
	}
	
	proc RBC.graph.marker.configure.bitmap.M.5.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring rotate changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.6.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.6.1.Body {} {
		.graph1 marker configure marker1 -rotate 30
	}
	
	proc RBC.graph.marker.configure.bitmap.M.6.1.Cleanup {} {
		destroy .graph1
	}
	
	# ------------------------------------------------------------------------------------
	# Purpose: Ensure configuring anchor changes on screen
	# ------------------------------------------------------------------------------------
	proc RBC.graph.marker.configure.bitmap.M.7.1.Setup {} {
		graph .graph1
		pack .graph1
		.graph1 marker create bitmap -name marker1 -bitmap @greenback.xbm -coords {0 0}
	}
	
	proc RBC.graph.marker.configure.bitmap.M.7.1.Body {} {
		.graph1 marker configure marker1 -anchor n
	}
	
	proc RBC.graph.marker.configure.bitmap.M.7.1.Cleanup {} {
		destroy .graph1
	}
    # ------------------------------------------------------------------------------------
    # Purpose: Ensure configuring a bitmap mask makes pixels outside the mask transparent.
    # ------------------------------------------------------------------------------------
    proc RBC.graph.marker.configure.bitmap.M.8.1.Setup {} {
        graph .graph1 -width 300 -height 300 -plotbackground red

        .graph1 axis configure x -min 0 -max 1
        .graph1 axis configure y -min 0 -max 1

        pack .graph1

        .graph1 marker create bitmap -name marker1 -bitmap @circle.xbm -foreground black -fill white -coords {0.5 0.5}
    }

    proc RBC.graph.marker.configure.bitmap.M.8.1.Body {} {
        .graph1 marker configure marker1 -mask @circle_mask.xbm
    }

    proc RBC.graph.marker.configure.bitmap.M.8.1.Cleanup {} {
        destroy .graph1
    }

    # ------------------------------------------------------------------------------------
    # Purpose: Ensure the mask remains aligned when the bitmap is scaled and rotated.
    # ------------------------------------------------------------------------------------
    proc RBC.graph.marker.configure.bitmap.M.8.2.Setup {} {
        graph .graph1 -width 300 -height 300 -plotbackground red

        .graph1 axis configure x -min 0 -max 1
        .graph1 axis configure y -min 0 -max 1

        pack .graph1

        .graph1 marker create bitmap -name marker1 -bitmap @circle.xbm -mask @circle_mask.xbm -foreground black -fill\
                white -coords {0.5 0.5}
    }

    proc RBC.graph.marker.configure.bitmap.M.8.2.Body {} {
        .graph1 marker configure marker1 -coords {0.25 0.25 0.75 0.75} -rotate 30
    }

    proc RBC.graph.marker.configure.bitmap.M.8.2.Cleanup {} {
        destroy .graph1
    }
}
